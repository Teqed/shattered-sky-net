		...
		const initScene = () => {
			...
			const initMeshes = () => {
				const initBody = (
					...
					const body = new RAPIER.RigidBody({
						mass: mass || 0,
						position,
						// shape: shape || new CANNON.Box(new CANNON.Vec3(1, 1, 1)),
						shape: shape || new RAPIER.Cuboid(1, 1, 1),
					})
					...
					const randomAxis = new RAPIER.Vector(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5)
					const update = () => {
				...
				const box = (mass?: number, position?: RAPIER.Vector, size?: RAPIER.Vector) => {
					...
					return initBody(
						...
						mesh, mass, position, new RAPIER.Cuboid(size?.x || 1, size?.y || 1, size?.z || 1)
					);
				}
